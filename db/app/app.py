            ...
            if get_metadata:
                ...
                for doc_id in pmids:
                    ...
                    if doc_id is not None:
                        ...
                        if insert_mysql:
                            query = (
                                'INSERT INTO searches (patid, docid, title, abs, ftext) VALUES (%s,%s,"%s","%s","%s") ON DUPLICATE KEY UPDATE title="%s", abs="%s", ftext="%s";'
                                % (
                                    pattern["id"],
                                    doc_id,
...
def pipeline2():
    ...
    for pattern in results:
        ...
        if pattern["db"] == "PUBMED":
            ...
            try:
                search_json = post_json_request(
                    "http://core:5000/core2",
                    {"query": pattern["pattern"], "idpattern": pattern["id"]},
                )
            except:
                get_metadata = False
                print("Not Success")
    ...
    results = execute_mysql_query(
        "SELECT patid, docid, title FROM searches order by patid desc",
        connection,
    )
    connection.close()
    return jsonify(results)
